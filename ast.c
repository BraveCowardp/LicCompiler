...
void display(struct node *T,int indent)  {//对抽象语法树的先根遍历
  ...
  case SWITCH:        printf("%*cswitch语句：\n",indent,' ');
                      printf("%*c选择条件：\n",indent+3,' ');
                      display(T->ptr[0],indent+6);      //显示选择条件
                      printf("%*c选择体：\n",indent+3,' ');
                      display(T->ptr[1],indent+6);      //显示选择体
                      break;
  case SWITCH_CASE:   printf("%*ccase语句：\n",indent,' ');
                      printf("%*ccase值：%d\n",indent+3,' ',T->type_int);
                      printf("%*ccase体：\n",indent+3,' ');
                      display(T->ptr[0],indent+3);
                      display(T->ptr[1],indent);
                      break;
  case SWITCH_DEFAULT:  printf("%*cdefault语句：\n",indent,' ');
                        printf("%*cdefault体：\n",indent+3,' ');
                        display(T->ptr[0],indent+3);
                        break;
	...
	case ID:	        printf("%*cID： %s\n",indent,' ',T->type_id);
                        break;
  case ARRAY_ID:    printf("%*c数组ID： %s\n",indent,' ',T->type_id);
                    display(T->ptr[0],indent+3);
                    break;
  case ARRAY_SIZE_LIST: printf("%*cARRAY_SIZE：%d\n",indent,' ',T->type_int);
                        display(T->ptr[0],indent);
                        break;
  case ARRAY_SIZE:  printf("%*cARRAY_SIZE：%d\n",indent,' ',T->type_int);
                    break;
  case ARRAY_UID: printf("%*cARRAY_UID： %s\n",indent,' ',T->type_id);
                  printf("%*cARRAY_INDEX：\n",indent+3,' ');
                  display(T->ptr[0],indent+3);
                  break;
