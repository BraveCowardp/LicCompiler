...
//  %type 定义非终结符的语义值类型
%type  <ptr> program ExtDefList ExtDef  Specifier ExtDecList FuncDec CompSt VarList VarDec ParamDec Stmt StmList switchStmt DefList Def DecList Dec Exp Args list Var deflist

//% token 定义终结符的语义值类型
%token <type_int> INT              //指定INT的语义值是type_int，有词法分析得到的数值
%token <type_id> ID RELOP TYPE  //指定ID,RELOP 的语义值是type_id，有词法分析得到的标识符字符串

%token LP RP LB RB LC RC SEMI COMMA COLON DEFAULT   //用bison对该文件编译时，带参数-d，生成的exp.tab.h中给这些单词进行编码，可在lex.l中包含parser.tab.h使用这些单词种类码
%token PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN SWITCH CASE AUTOADD AUTOSUB BREAK
...
ExtDef:   Specifier ExtDecList SEMI   {$$=mknode(EXT_VAR_DEF,$1,$2,NULL,yylineno);}   //该结点对应一个外部变量声明
         ...
         ;
Specifier:  TYPE    {$$=mknode(TYPE,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);$$->type=INT;}   
           ;      
...
VarDec:  ID          {$$=mknode(ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
         |ID deflist {$$=mknode(ARRAY_ID,$2,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
         ;
deflist: LB INT RB deflist {$$=mknode(ARRAY_SIZE_LIST,$4,NULL,NULL,yylineno);$$->type_int=$2;$$->type=INT;}
        | LB INT RB {$$=mknode(ARRAY_SIZE,NULL,NULL,NULL,yylineno);$$->type_int=$2;$$->type=INT;}
        ;
...
Stmt:   Exp SEMI    {$$=mknode(EXP_STMT,$1,NULL,NULL,yylineno);}
      ...
      | SWITCH LP Exp RP LC switchStmt RC {$$=mknode(SWITCH,$3,$6,NULL,yylineno);}
      | BREAK SEMI {$$=mknode(BREAK,NULL,NULL,NULL,yylineno);}
      ;
switchStmt: CASE INT COLON StmList switchStmt {$$=mknode(SWITCH_CASE,$4,$5,NULL,yylineno);$$->type_int=$2;$$->type=INT;}
          | DEFAULT COLON StmList {$$=mknode(SWITCH_DEFAULT,$3,NULL,NULL,yylineno);}
          ;
DefList: {$$=NULL; }
...
Def:    Specifier DecList SEMI {$$=mknode(VAR_DEF,$1,$2,NULL,yylineno);}
        ;
DecList: Dec  {$$=mknode(DEC,$1,NULL,NULL,yylineno);}
       | Dec COMMA DecList  {$$=mknode(DEC_LIST,$1,$3,NULL,yylineno);}
	   ;
Dec:     VarDec  {$$=$1;}
       ;
Var:    ID {$$=mknode(ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
      | ID list {$$=mknode(ARRAY_UID,$2,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
list:   LB Exp RB list {$$=mknode(ARRAY_INDEX_LIST,$2,$4,NULL,yylineno);}
      | LB Exp RB {$$=$2;}
